...
import pt.estg.leaflings.handler.CreateRewardServiceHandler;

import java.time.Duration;

@SpringBootApplication
public class LeaflingsApplication {
	...
	public static void main(String[] args) {
		...
		try (final ZeebeClient client =
			...
			client.newCreateInstanceCommand()
					.bpmnProcessId("RewardsProcess")
					.latestVersion()
					.send()
					.join();

			final JobWorker createNewReward =
					client.newWorker()
							.jobType("External")
							.handler(new CreateRewardServiceHandler())  // Implementação do handler
		...
		SpringApplication.run(LeaflingsApplication.class, args);
	}
